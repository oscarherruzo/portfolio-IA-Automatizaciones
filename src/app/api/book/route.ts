...
function buildSlots(schedule: any, serviceDuration: number, date: string, booked: any[]) {
  ...
  const workDays = schedule.working_days || [1,2,3,4,5];
  if (!workDays.includes(dow)) return [];

  const now = new Date();
  const isToday = date === now.toISOString().split("T")[0];
  const [sh, sm] = (schedule.start_time || "09:00").split(":").map(Number);
  const [eh, em] = (schedule.end_time   || "19:00").split(":").map(Number);
  ...
  for (let m = sh*60+sm; m+serviceDuration <= eh*60+em; m += slotDur) {
    ...
    if (isToday) {
      const t = new Date(`${date}T${timeStr}:00`);
      if (t <= now) continue;
    }
    ...
    if (!occupied) slots.push(timeStr);
  }
  return slots;
}
